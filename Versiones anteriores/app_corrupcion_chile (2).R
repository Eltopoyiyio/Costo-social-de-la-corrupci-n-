...
tabla_subareas <- tibble::tribble(
  ...
  "Pensiones",  "pensiones", "Trabajadores informales sin cotización",          "jovenes_cot",       0.6,                 "trabajadores informales con un año de cotización","Escenario proyectado", 1980000,     "Costo de garantizar un año de cotizaciones a los 3,3M de trabajadores informales",
...
impactos_texto <- list(
  ...
  becas = list(
    inverlink = "Los $200.940 millones sustraídos de CORFO habrían financiado una beca universitaria anual para 174.730 estudiantes —el doble de los 85.000 que hoy cumplen todos los requisitos pero se quedan sin asignación porque el presupuesto se agota. Con ese dinero el Estado habría podido becar a cada uno de los estudiantes elegibles que se quedaron sin beca en más de dos años consecutivos.",
    pacogate  = "Los $41.898 millones malversados en Carabineros habrían financiado una beca universitaria anual para 36.433 estudiantes elegibles que hoy se quedan sin asignación. Son más de un tercio de los 85.000 que cada año cumplen todos los requisitos y no reciben la beca porque el presupuesto se agotó antes de llegar a su postulación.",
    sqm       = "Los $13.500 millones que SQM no tributó habrían financiado una beca universitaria anual para 11.739 estudiantes que cumplen los requisitos pero no reciben asignación. Más de once mil jóvenes que el Estado ya evaluó, ya calificó como elegibles, y a quienes simplemente no alcanzó el presupuesto.",
    penta     = "Los $16.200 millones del Caso Penta habrían financiado una beca universitaria anual para 14.087 estudiantes elegibles sin asignación —el 16,5% del total de jóvenes que cada año quedan fuera del sistema de becas sin haber reprobado ningún criterio. El Grupo Penta pagó multas y cumplió condenas. Esos 14.000 estudiantes no tienen multa que pagar ni condena que cumplir para recibir lo que les corresponde.",
    milicogate = "Los $10.657 millones desviados de los fondos del Ejército habrían financiado una beca universitaria anual para 9.267 estudiantes que cumplieron todos los requisitos y no recibieron asignación. Casi 9.300 jóvenes que el propio sistema reconoció como elegibles y a quienes el presupuesto no alcanzó."
  ),
...
svg_simbolo <- function(area, fill_color, stroke_color, fill_pct = NULL, show_ghost = FALSE) {
  ...
  shapes <- switch(area,
                   ...
                   "Pensiones" = {
                     # Coin symbol (thick, solid)
                     paste0(
                       # outer circle
                       '<circle cx="100" cy="110" r="88" fill="', ghost_fill, '" stroke="', ghost_stroke, '" stroke-width="6" opacity="', ghost_opacity, '"/>',
                       # inner circle ring
                       '<circle cx="100" cy="110" r="68" fill="none" stroke="', ghost_stroke, '" stroke-width="5" opacity="', ghost_opacity, '"/>',
                       # $ symbol vertical bar top
                       '<line x1="100" y1="48" x2="100" y2="68" stroke="white" stroke-width="8" stroke-linecap="round" opacity="0.9"/>',
                       # $ symbol vertical bar bottom
                       '<line x1="100" y1="152" x2="100" y2="172" stroke="white" stroke-width="8" stroke-linecap="round" opacity="0.9"/>',
                       # $ symbol — S curve (top arc)
                       '<path d="M120,75 Q138,75 138,92 Q138,110 100,110" fill="none" stroke="white" stroke-width="10" stroke-linecap="round" opacity="0.9"/>',
                       # $ symbol — S curve (bottom arc)
                       '<path d="M80,145 Q62,145 62,128 Q62,110 100,110" fill="none" stroke="white" stroke-width="10" stroke-linecap="round" opacity="0.9"/>'
                     )
                   },
  ...
  if (!is.null(fill_pct) && !show_ghost) {
    ...
    ghost_shapes <- switch(area,
                           ...
                           "Pensiones"  = paste0('<circle cx="100" cy="110" r="88" fill="#D3D1C7" stroke="#B4B2A9" stroke-width="6"/><circle cx="100" cy="110" r="68" fill="none" stroke="#B4B2A9" stroke-width="5"/><line x1="100" y1="48" x2="100" y2="68" stroke="white" stroke-width="8" stroke-linecap="round" opacity="0.9"/><line x1="100" y1="152" x2="100" y2="172" stroke="white" stroke-width="8" stroke-linecap="round" opacity="0.9"/><path d="M120,75 Q138,75 138,92 Q138,110 100,110" fill="none" stroke="white" stroke-width="10" stroke-linecap="round" opacity="0.9"/><path d="M80,145 Q62,145 62,128 Q62,110 100,110" fill="none" stroke="white" stroke-width="10" stroke-linecap="round" opacity="0.9"/>'),
